...
//Picked one of the account created by the mnemonic in 
console.log("private key: ", process.env.WALLET_PRIVATE_KEY);
// creates the wallet, also connects the wallet to a provider
const signerWallet = new ethers.Wallet(process.env.WALLET_PRIVATE_KEY, provider);
// what I expect to see in logs: process.env.WALLET_PUBLIC_ADDRESS
console.log("My wallet address derived from private key is:", signerWallet.address);
...
let signerBalance =  await provider.getBalance(signerWallet.address);
console.log(
    "Sepolia Balance",
